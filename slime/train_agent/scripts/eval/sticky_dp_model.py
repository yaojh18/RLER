"""Sticky-DP wrapper around mini-swe-agent's LitellmTextbasedModel.
...
in one rank's radix KV cache.

mini-swe-agent calls `get_model(...)` once per benchmark instance inside
`process_instance`, so a global round-robin counter at __init__ time gives
each instance a stable rank for the whole trajectory.
