...
case "${METHOD:-}" in
  baseline|judge_direct_rollout40|judge_direct_depth2)
    ;;
  *)
    echo "ERROR: METHOD must be baseline|judge_direct_rollout40|judge_direct_depth2" >&2
    return 2
    ;;
esac
...
# Hosted direct Lane-C is independent of the local 64K Qwen policy service.
RLER_HOSTED_MODEL_CONTEXT_LENGTH="${RLER_HOSTED_MODEL_CONTEXT_LENGTH:-256000}"
...
SWE_AGENT_LANES_JUDGE_MODEL="${SWE_AGENT_LANES_JUDGE_MODEL:-openai/azure/openai/gpt-5.6-luna}"
_EARLYPRED_CONFIG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
_RLER_ROOT="$(cd "$_EARLYPRED_CONFIG_DIR/../../.." && pwd)"
SWE_AGENT_LANES_DIRECT_RUBRIC_BANK="${SWE_AGENT_LANES_DIRECT_RUBRIC_BANK:-$_RLER_ROOT/rubric/qwen/golden_reference_rubrics.json}"
SWE_AGENT_LANES_ENABLE_VARIANCE_DETECTOR="${SWE_AGENT_LANES_ENABLE_VARIANCE_DETECTOR:-1}"
SWE_AGENT_LANES_COLLAPSE_REWARD_MARGIN="${SWE_AGENT_LANES_COLLAPSE_REWARD_MARGIN:-0.5}"

# Do not serialize or globally pace Lane-C calls. Every frozen rubric x branch
# judge call is independent; exhausted required calls invalidate only the
# affected GRPO group.
...
if [ -z "${SLIME_WANDB_METRIC_ALLOWLIST:-}" ]; then
  ...
  case "$METHOD" in
    baseline)
      ...
      ;;
    judge_direct_rollout40|judge_direct_depth2)
...
export SWE_AGENT_LANES_JUDGE_MODEL
export SWE_AGENT_LANES_DIRECT_RUBRIC_BANK
export SWE_AGENT_LANES_ENABLE_VARIANCE_DETECTOR
export SWE_AGENT_LANES_COLLAPSE_REWARD_MARGIN
