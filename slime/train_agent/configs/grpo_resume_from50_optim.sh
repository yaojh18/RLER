# Like grpo_resume_from50.sh but drops --no-load-optim/--no-load-rng/--finetune
# so Megatron loads optimizer state + RNG + LR scheduler from the ckpt.
# Use when LOAD_DIR points at a ckpt saved by an earlier slime training
# (i.e. distcp shards contain Adam m/v + fp32 master weights, not just
# converted HF weights). Pairs with --start-rollout-id 50 already in the
# parent file so slime continues rollouts at step 50.
GRPO_COMMON_ARGS=(
  --save-interval 10
  --start-rollout-id 50
